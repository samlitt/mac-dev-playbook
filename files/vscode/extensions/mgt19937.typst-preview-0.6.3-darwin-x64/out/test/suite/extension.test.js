...
const jsonIs = (pred) => (x, y) => pred(JSON.stringify(x), JSON.stringify(y));
suite('Extension Test Suite', () => {
    ...
    test('Executable Configuration Test', async () => {
        assert.strictEqual('', vscode.workspace.getConfiguration().get('typst-preview.executable'), 'default path');
        ...
        const state = ext.getTypstWsPath;
        let resolved;
    ...
    test("FontPaths Configuration Test", async () => {
        ...
        jsonIs(assert.strictEqual)([], vscode.workspace.getConfiguration().get("typst-preview.fontPaths"));
        jsonIs(assert.strictEqual)([], ext.getTypstWsFontArgs(undefined));
        jsonIs(assert.strictEqual)([], ext.getTypstWsFontArgs([]));
        jsonIs(assert.strictEqual)([], ext.codeGetTypstWsFontArgs());
        jsonIs(assert.strictEqual)(["--font-path", "/path/to/font1", "--font-path", "/path/to/font2"], ext.getTypstWsFontArgs(["/path/to/font1", "/path/to/font2"]));
    });
});
