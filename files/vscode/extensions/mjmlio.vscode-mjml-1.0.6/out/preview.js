...
class Preview {
    constructor(context) {
        ...
        this.subscriptions.push(vscode_1.commands.registerCommand('mjml.previewToSide', () => {
            ...
            if (document &&
                this.previewOpen &&
                vscode_1.workspace.getConfiguration('mjml').autoPreview) {
            ...
            if (event &&
                this.previewOpen &&
                vscode_1.workspace.getConfiguration('mjml').updateWhenTyping) {
            ...
            if (document && this.previewOpen && this.webview) {
                ...
                if (this.openedDocuments.length === 0 &&
                    vscode_1.workspace.getConfiguration('mjml').autoClosePreview) {
                    this.dispose();
                }
            }
        }));
    ...
    getContent(document) {
        if (!vscode_1.workspace.getConfiguration('mjml').switchOnSeparateFileChange) {
            document = this.openedDocuments[0] || document;
        }
        const html = helper_1.mjmlToHtml(this.wrapInMjmlTemplate(document.getText()), false, false, document.uri.fsPath, 'skip').html;
        if (html) {
    ...
    wrapInMjmlTemplate(documentText) {
    ...
    removeDocument(fileName) {
        this.openedDocuments = this.openedDocuments.filter((file) => file.fileName !== fileName);
    }
}
