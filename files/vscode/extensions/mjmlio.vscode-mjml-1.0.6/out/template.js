...
class Template {
            ...
            this.templateList = yield vscode_1.window.withProgress({
                ...
                return yield tree.reduce((map, item) => {
                    ...
                    if (!item.path.startsWith('templates/') &&
                        !item.path.startsWith('thumbnails/') &&
                        !extract) {
                        return map;
                    }
    ...
    quickPick() {
        vscode_1.window
            .showQuickPick(Object.keys(this.templateList), {
    ...
    createFile(templateURL, activeEditor) {
        node_fetch_1.default(templateURL)
            .then((response) => {
            ...
            if (body) {
                if (activeEditor) {
                    ...
                    if (vscode_1.workspace.getConfiguration('mjml').templateGalleryAutoClose &&
                        this.webview) {
