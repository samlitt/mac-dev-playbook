...
class Documentation {
    ...
    displayWebView() {
        if (!this.webview) {
            ...
            if (!documentationPath ||
                !fs_1.existsSync(documentationPath) ||
                !fs_1.statSync(documentationPath).isFile()) {
                return;
            }
            this.webview = vscode_1.window.createWebviewPanel('mjml-documentation', 'MJML Documentation', vscode_1.ViewColumn.Two, {
                enableFindWidget: true,
                enableScripts: true,
                localResourceRoots: [vscode_1.Uri.parse(this.context.extensionPath)],
    ...
    getWebviewContent(filePath) {
        if (!this.content) {
            const rootPath = vscode_1.Uri.parse(path_1.join(this.context.extensionPath, 'documentation'))
                .with({
                scheme: 'vscode-resource',
            })
                .toString();
            this.content = fs_1.readFileSync(filePath)
                .toString()
