...
suite('Extension Test Suite', () => {
    ...
    [
        ...
        test('authority name is the class name on the active line', async () => {
            const uri = vscode.Uri.file(path.join(__dirname + fixturesLocation + 'test_example.py'));
            const document = await vscode.workspace.openTextDocument(uri);
            const selection = new vscode.Selection(data.line, 0, data.line, 0);
            const authName = Ext.getTestAuthorityName(data.checkCfg, data.cfg, selection, document);
    ...
    [
        ...
        test('authority name is the text selection', async () => {
            const uri = vscode.Uri.file(path.join(__dirname + fixturesLocation + 'test_example.py'));
            const document = await vscode.workspace.openTextDocument(uri);
            const selection = new vscode.Selection(data.selection.line, data.selection.anchorChar, data.selection.line, data.selection.activeChar);
            const authName = Ext.getTestAuthorityName(data.checkCfg, data.cfg, selection, document);
            assert.strictEqual(authName, data.expected);
        });
    });
